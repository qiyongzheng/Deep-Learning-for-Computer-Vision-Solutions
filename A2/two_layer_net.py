...
def sample_batch(X, y, num_train, batch_size):
  ...
  N = X.shape[0]
  indices = torch.randint(N - 1, (batch_size,))
  X_batch = X[indices]
  y_batch = y[indices]
  #########################################################################
  #                       END OF YOUR CODE                                #
...
def nn_forward_backward(params, X, y=None, reg=0.0):
    ...
    Y = torch.eye(scores.shape[1], device=W1.device, dtype=P.dtype)
    Y = Y[y]

    correct_probs = torch.diag(torch.mm(P, Y.T))

    loss = -torch.mean(torch.log(correct_probs)) 
    loss += reg * (torch.sum(W1 ** 2) + torch.sum(W2 ** 2))
    # loss = -torch.mean(torch.log(P[torch.arange(N), y])) 
    # loss += reg * (torch.sum(W1 ** 2) + torch.sum(W2 ** 2))
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    ...
    grads['W2'], grads['W1'], grads['b2'], grads['b1'] = dW2, dW1, db2, db1
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def nn_get_search_params():
  ...
  ###########################################################################
  # Replace "pass" statement with your code
  learning_rates = torch.linspace(1e-2, 1.5, steps=10).numpy()
  hidden_sizes = [4096, 8192]
  regularization_strengths = [0, 1e-4]
  learning_rate_decays = [1, 0.99]
  ###########################################################################
  #                           END OF YOUR CODE                              #
  ###########################################################################
    ...
    for hs in hidden_sizes:
        for reg in regularization_strengths:
            for lrd in learning_rate_decays:
                ...
                stats = net.train(
                  data_dict['X_train'], data_dict['y_train'], data_dict['X_val'], data_dict['y_val'],
                  num_iters=3000, batch_size=1000,
                  learning_rate=lr, learning_rate_decay=lrd,
                  reg=reg, verbose=False)

                val_acc = max(stats['val_acc_history'])
                result_dict[val_acc] = [lr, hs, reg, lrd]
                print(f'{val_acc:.5f}: {lr:.5f}, {hs}, {reg:.4f}, {lrd}')
  
  best_val_acc = max(result_dict)
  lr, hs, reg, lrd = result_dict[best_val_acc]
  best_net = TwoLayerNet(3 * 32 * 32, hs, 10, device=data_dict['X_train'].device, dtype=data_dict['X_train'].dtype)
  best_stat = best_net.train(
    data_dict['X_train'], data_dict['y_train'], data_dict['X_val'], data_dict['y_val'],
    num_iters=3000, batch_size=1000,
    learning_rate=lr, learning_rate_decay=lrd,
    reg=reg, verbose=False)
  
  print(f'best_net: {lr:.5f}, {hs}, {reg:.4f}, {lrd}')
  #############################################################################
  #                               END OF YOUR CODE                            #
  #############################################################################
