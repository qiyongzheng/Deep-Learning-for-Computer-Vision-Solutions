...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None):
    ...
    dhidden, grads['W2'], grads['b2'] = Linear.backward(dscores, cache_2)
...
class FullyConnectedNet(object):
  ...
  def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
    ...
    for i in range(2, self.num_layers):
      input_dim = hidden_dims[i - 2]
      output_dim = hidden_dims[i - 1]
      self.params[f'W{i}'] = weight_scale * torch.randn(input_dim, output_dim, 
                                                        dtype=dtype, device=device)
      self.params[f'b{i}'] = torch.zeros(output_dim, dtype=dtype, device=device)
    
    self.params[f'W{self.num_layers}'] = weight_scale * torch.randn(hidden_dims[-1], 
                                                                    num_classes, 
                                                                    dtype=dtype, 
                                                                    device=device)
  ...
  def loss(self, X, y=None):
    ...
    # len = num_layers - 1
    cache_history = []
    dropout_cache_history = []

    for i in range(1, self.num_layers):
      W = self.params[f'W{i}']
      b = self.params[f'b{i}']
      hidden, cache = Linear_ReLU.forward(X_vec, W, b)
      if self.use_dropout:
        hidden, dropout_cache = Dropout.forward(hidden, self.dropout_param)
        dropout_cache_history.append(dropout_cache)
      X_vec = hidden
    ...
    loss, dscores = softmax_loss(scores, y)
    sum_norm = sum([self.params[f'W{i}'].pow(2).sum()
                    for i in range(1, self.num_layers + 1)])
    loss += self.reg * sum_norm
   
    ## grads
    dhidden, grads[f'W{self.num_layers}'], grads[f'b{self.num_layers}'] = Linear.backward(dscores, cache)
    grads[f'W{self.num_layers}'] += self.reg * 2 * self.params[f'W{self.num_layers}']

    for i in range(self.num_layers - 1, 0, -1):
      cache = cache_history[i - 1]
      if self.use_dropout:
        dropout_cache = dropout_cache_history[i - 1]
        dhidden = Dropout.backward(dhidden, dropout_cache)
      dhidden, dW, db = Linear_ReLU.backward(dhidden, cache)
      dW += 2 * self.reg * self.params[f'W{i}']
      grads[f'W{i}'] = dW
      grads[f'b{i}'] = db
    ###########################################################################
    ############################################################################
