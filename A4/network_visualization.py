...
def compute_saliency_maps(X, y, model):
  ...
  scores = model(X)
  correct_scores = scores.gather(1, y.reshape(-1, 1)).squeeze()
...
def make_adversarial_attack(X, target_y, model, max_iter=100, verbose=True):
  ...
  for _ in range(max_iter):
    model.zero_grad()
    scores = model(X_adv).squeeze()
    correct_score = scores[target_y]
    if scores.max() == correct_score:
      break
    correct_score.backward()
    with torch.no_grad():
      g = X_adv.grad
      dX = learning_rate * g / g.norm(2)
      X_adv += dX
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
...
def class_visualization_step(img, target_y, model, **kwargs):
    ...
    opt_fn = target_score - l2_reg * img.norm(2)
    opt_fn.backward()
    with torch.no_grad():
